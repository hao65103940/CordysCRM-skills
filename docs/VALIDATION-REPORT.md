...
| # | 场景 | 状态 | 备注 |
|---|------|------|------|
| 1 | 查询线索列表 | ✅ 通过 | 返回 8664 条线索 |
| 2 | 全局搜索客户 | ✅ 通过 | 搜索"科技"返回 30+ 结果 |
| 3 | 按区域过滤线索 | ✅ 通过 | 东区线索正常过滤 |
| 4 | 按时间范围查询 | ✅ 通过 | 本周创建 159 条线索 |
| 5 | 查询已成交商机 | ✅ 通过 | 返回 SUCCESS 阶段商机 |
| 6 | 按产品过滤商机 | ✅ 通过 | JumpServer 商机正常 |
...
```json
{
  "name": "国旅运通-JS-2026-订阅续费",
  "amount": 46000,
  "stage": "SUCCESS"
}
{
  "name": "Ridger Company Limited-OpenClaw 一体机 x2-2026",
  "amount": 42000,
  "stage": "SUCCESS"
}
{
  "name": "威海临港-MK&Openclaw-2026-一体机新购",
  "amount": 65000,
  "stage": "SUCCESS"
}
```
...
{
  "name": "国旅运通-JS-2026-订阅续费",
  "customerName": "国旅运通航空服务有限公司",
  "amount": 46000
}
```

**验证结果：** ✅ 通过（JumpServer 成单客户）
